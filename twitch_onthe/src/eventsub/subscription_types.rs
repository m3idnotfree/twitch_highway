...
macro_rules! subscription_types {
    (
        $(#[$meta:meta])*
        enum SubscriptionType {
            $($name:ident ($lit:literal, $ver:literal),)+
        }
    ) => {
        ...
        impl SubscriptionType {
            ...
            pub fn from_type_and_version(sub_type: &str, version: Option<&str>) -> Result<Self, String> {
                match (sub_type, version){
                    ("automod.message.hold", Some("2")) => Ok(Self::AutomodMessageHoldV2),
                    ("automod.message.update", Some("2")) => Ok(Self::AutomodMessageUpdateV2),
                    ("channel.moderate",Some("2")) => Ok(Self::ChannelModerateV2),
                    ("channel.channel_points_automatic_reward_redemption.add", Some("2")) => {
                        Ok(Self::ChannelPointsAutomaticRewardRedemptionAddV2)
                    }
                    $(($lit, _) => Ok(Self::$name),)+
                    _ => Err(format!("unknown subscription type: '{sub_type}'")),
                }
            }
        }
